...
class CastTab(ttk.Frame):
    ...
    def _write_guide(self) -> Path:
        ...
        body = (
            ...
            # ツール名を決め打ちせず、読んだ AI が自分の環境に合わせて動けるようにする
            "## 読み上げさせ方（自分に当てはまる経路を使ってください）\n\n"
            "**A. 応答完了フック導入済みの場合** — 何もする必要はありません。\n"
            "返答に記法（下記）で書いたセリフを含めると、応答の完了時に自動で読み上げられます。\n"
            "読み上げは「返答の一部を声にする」仕組みです。返答とは別に読み上げ用のテキストを\n"
            "作らないでください（書いた文と違うことを声が言うのは望まれていません）。\n"
            "お使いのツールのフック設定に `--hook` を含む登録があれば導入済みです。\n\n"
            "**Codex デスクトップアプリを使う場合** — 返答本文を `A『…』` の記法で\n"
            "先に完成させ、その同じ本文を下記の `--bg` へ渡すよう Codex に伝えてください。\n"
            "読み上げ専用の別原稿を作らせないことが重要です。Stop フックは、実行と信頼承認を\n"
            "確認できた環境でだけ自動化に使い、未確認なら前提にしないでください。\n\n"
            f"**B. フック未導入の場合** — お使いのツールに応答完了フック（応答が完了したときに\n"
            f"コマンドを実行する仕組み）があれば、`{cmd} --hook` を登録してください。\n"
            "標準入力からアシスタントの応答を含む JSON を受け取り、記法のセリフを自動で\n"
            "読み上げます。登録は一度きりで、次回以降のセッションから有効です。\n"
            "フック非対応の環境では C の方法をお使いください。\n\n"
            "**C. フック非対応の環境や、他のソフト・スクリプトから使う場合** —\n"
            "下記の CLI か HTTP で直接読み上げできます。\n"
